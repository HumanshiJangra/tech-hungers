...
import { title } from 'process';
 ...
 const service = [
    ...
    { id: 6, title: "Web Maintenance", subtitle: "Keep your Website Maintenance always a top priority for Peak Performance.", image: image2}
  ];
    ...
    <div className="container"> 
      ...
      <div className="grid grid-cols-3 gap-5 mt-10">
        {service.map((item, index) => (
          <div key={index} className="p-8 bg-[radial-gradient(at_top_left,_#212542_0%,_#15172a_100%)] border border-[#232746] rounded-[20px] hover:border-[#535989] ">
            <Image
              src={item.image}
              alt={item.title}
              width={50}
            ...
            <button onClick={()=> openService(item.id)} className='text-[#99a5ff] flex items-center gap-2 '>learn More <MoveRight color='#99a5ff'/> </button>
          </div>
        ))}
        {/* <div className='grid place-items-center p-8 text-center'>
          ...
          <Button href='/services' text='View More' />
        </div> */}
      </div>
    </div>
  </div>
